...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    user_name = object
    for tweet in tweets:
        if tweet.retweets / tweet.time > result:
            result = tweet.retweets / tweet.time
            user_name = tweet
    return user_name
...
def sort_by_popularity(tweets: list) -> list:
    ...
    """
    return sorted(tweets, key=lambda x: (-x.retweets, x.time))
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    hashtag_retweets = {}
    for tweet in tweets:
        match = re.findall(pattern, tweet.content)
        if len(match) > 0:
            for hashtag in match:
                hashtag_retweets.setdefault(hashtag, []).append(tweet.retweets)
    hashtag_list = list(hashtag_retweets.keys())
    sorted_tweets = sorted(hashtag_list, key=lambda hashtag_name: (-sum(hashtag_retweets[hashtag_name]), hashtag_name))
...
if __name__ == '__main__':
    ...
    tweet3 = Tweet("@CIA", "We can neither confirm nor deny that this is our first tweet. #ZEart", 2192, 220802)
